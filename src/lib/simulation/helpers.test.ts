...
describe("drawFromAccounts", () => {
  it("withdrawalOrder順に取り崩す", () => {
    const accts = createMemberAccounts({ nisa: 100, tokutei: 200, cash: 50 });
  ...
  it("deficitより残高が少ない場合は全額取り崩し", () => {
    const accts = createMemberAccounts({ tokutei: 100 });
  ...
  it("iDeCoは60歳未満で取り崩し不可", () => {
    const accts = createMemberAccounts({ ideco: 1000 });
  ...
  it("tokutei取り崩し時にcostBasisが按分減少", () => {
    ...
    const accts = createMemberAccounts({ tokutei: 1000, tokuteiCB: cb });
  ...
  it("gold取り崩し時にcostBasisが按分減少", () => {
    ...
    const accts = createMemberAccounts({ gold: 2000, goldCB: cb });
...
describe("contributeSurplus", () => {
  it("NISA枠ありならNISA優先→tokutei", () => {
    const accts = createMemberAccounts();
  ...
  it("NISA生涯枠を超えたらtokuteiのみ", () => {
    const accts = createMemberAccounts({ nisaCumulative: 18_000_000 });
  ...
  it("積立時にtokuteiCBのcostが増加", () => {
    ...
    const accts = createMemberAccounts({ tokuteiCB: cb });
