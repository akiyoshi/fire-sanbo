...
| 機能 | 実装 | テスト |
|------|------|--------|
| モンテカルロシミュレーション | `simulation/engine.ts` runTrial | 54テスト |
| CostBasis取得費追跡 | `simulation/cost-basis.ts` | 14テスト |
| 口座操作ヘルパー | `simulation/helpers.ts` | 10テスト |
| 最悪ケース診断 | `simulation/diagnosis.ts` | 7テスト |
| 日本税制エンジン (2026年度) | `tax/engine.ts` | 43テスト |
| 処方箋 (4軸二分探索+フロンティア走査) | `prescription/engine.ts` runTrialLite | 22テスト |
...
- **4軸の処方箋**: 支出削減 / 退職延期 / 収入増加 / アロケーション最適化
- **収入増加軸**: `annualSalary` の二分探索で「年収を+X万円にする」を提案
- **アロケーション軸**: 効率的フロンティアを走査し「リスクをX%→Y%に調整」を提案、ワンクリック適用
- **年金・退職金・副収入・ライフイベント反映済み**
- **難易度タグ**: やさしい / ふつう / むずかしい
- **自動展開**: 成功率90%未満時に処方箋セクションを自動で開く
- **アクセントボーダー**: 最高インパクトの処方箋を視覚的に強調
...
- **口座別リターン**: 各口座(NISA/特定/iDeCo/金)の資産クラス構成から個別の期待リターン・リスクを導出、口座ごとに異なるリターンを適用
- **積立リバランス**: 退職前の余剰積立時、目標ウェイトに対して最も不足している口座に優先的に配分
...
- **リバランストグル**: ON/OFFで積立・退職後リバランスを有効化、OFF時は従来動作
- **目標→口座ウェイト変換**: 現在保有の口座内訳比率ベースで目標を口座レベルに分配
- **オプティマイザ連携**: 効率的フロンティアの推奨配分を「目標に設定」でtargetAllocationに直接適用
- **自動生成**: トグルON時に目標未設定なら現在のportfolioから資産クラスウェイトを自動導出
- **FormState v5**: `targetAllocation?: TargetAllocation[]` + `rebalanceEnabled?: boolean` 追加
...
- **15セクション・4グループ構成**: 収入と税金(5) / 資産の取り崩し(3) / シミュレーションの仕組み(4) / リバランスの仕組み(3)
...
- **FormState v5**: スキーマバージョン管理 + v2→v3→v4→v5マイグレーション。v4で `withdrawalOrder`、v5で `targetAllocation` + `rebalanceEnabled` 追加
...
- **配列長上限**: portfolio≤20件、lifeEvents≤30件、targetAllocation≤20件 (O(n²) DoS防御)
- **targetAllocationバリデーション**: weight型チェック + 0-1クランプ（共有URL/JSONインポート経由の不正値防御）
...
v4.3で目標アロケーション + リバランスUIを完成し、実質完成版とする。以下のみ対応:
- 年次の税制JSON更新
- 依存関係のセキュリティパッチ
- バグ修正
以下は凍結:
