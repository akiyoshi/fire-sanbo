...
- [ ] **名前遷移**: FIRE参謀 → 資産参謀 (FIRE以外のユーザー層拡大時、URL維持戦略要検討)
- [ ] **動的OGP**: シミュレーション結果をOGP画像に反映 (Edge Function)
- [ ] **PWA対応**: オフライン実行、ホーム画面追加
- [ ] **FP向けデモ**: 独立系FP 5人に見せてBtoB温度感を測定

### Phase 6: 税制年度対応 + PWA (v4.0)

- `tax-config-2027.json` 追加 (毎年1月に手動更新)
- FormState v4: `fiscalYear` フィールド追加 + v3→v4マイグレーション
- 共有URLの後方互換: `fiscalYear` 未指定時は2026デフォルト
- 年度跨ぎ設計判断: **開始年度で全年固定** (将来の税制変更は反映しない旨を免責条項に明記)
- PWA: Vite PWAプラグイン + Service Worker + オフラインキャッシュ

### Phase 7: リブランド + BtoB検証 (v5.0)

- FIRE参謀 → 資産参謀 (名前遷移)
- GitHub Pages URL維持戦略: リポ名変更時の301リダイレクト or 404.htmlリダイレクト
- SEO移行期間: 両方の名前を `<meta description>` に含める
- OGP画像・favicon・メタデータ一括更新チェックリスト
- FP向けホワイトレーベル: 月額サブスク検証 (100人のFPが使い始めてから)

---

## 中長期戦略 (2026年4月 CEOレビュー)

### ビジョン再定義

FIRE参謀が解いている問題は「FIRE達成率の計算」ではない。
**「将来のお金の不安を、構造化された数字に変換すること」**。

| 現在のフレーム | 10倍のフレーム |
|--------------|--------------|
| FIREの成功率 | 人生の資産設計エンジン |
| 個人の試算 | 世帯の意思決定ツール |
| 静的な計算根拠書 | 動的な学習コンテンツ |
| URLで結果共有 | FPとの相談基盤 |
...
| 競合 | 強み | 弱み |
|------|------|------|
| マネーフォワード ME | 家計簿+資産管理 MAU 1,000万人超 | シミュレーションは簡易、税制反映なし |
| Financial Teacher System | FP向けプロ仕様 | 有料、UI古い、個人利用困難 |
| 金融庁ライフプランシミュレーション | 公式、信頼性 | 入力項目少なすぎ、NISA/iDeCo非対応 |
| cFIRE.jp / FIRE計算機 | FIRE特化 | 日本税制なし、4%ルールのみ |
| Projection Lab (US) | 美しいUI、多機能 | 日本非対応、英語、有料 |

**差別化 (堀/moat)**: 2026年度日本税制の正確なOSS実装 + 処方箋逆算エンジン + 完全クライアントサイド (プライバシー)。この3つを持つ競合はゼロ。

### ターゲットユーザー

最初の100人:
1. **30-40代 IT/金融系、年収600-1200万** — NISA/iDeCo枠使い切り勢。改善フィードバック提供
2. **FIREブロガー/YouTuber** — コンテンツのネタ＋リンク拡散（バイラル装置）
3. **独立系FP** — クライアント説明ツールとして転用（BtoB化の入口）

### 10倍体験

- **A) 対話型アドバイザー**: 「手取り35万、子供2人、持ち家。55歳退職」→ テンプレート自動選択 → 確認・微調整だけ
- **B) 人生のWhat-if**: 「転職で年収200万減」→ シナリオ差分プリセットで1クリック比較
- **C) 共有→議論→合意**: パートナーと同じシミュレーションを見ながら退職時期を議論。差分共有モデル

### 技術的リスク

| # | リスク | 影響度 | 緩和策 |
|---|--------|--------|--------|
| 1 | 税制改正の追従 | 高 | `tax-config-{year}.json` のバージョニング。年度切替ロジック設計済 |
| 2 | 計算精度への信頼 | 高 | 免責条項 + 12セクション計算根拠書 + 出典リンク |
| 3 | クライアントサイド限界 | 中 | 将来的にOptional サーバーサイドストレージ。「サーバーなし」がコアバリュー |

### 市場タイミング

- **新NISA (2024年〜)**: 「NISAで何をどう買うか」の関心爆発
- **退職金課税見直し議論 (2025年)**: 退職所得控除の計算需要急増
- **FIREムーブメント成熟**: 4%ルール → 日本税制考慮の精密計算へシフト
- **AI時代**: クライアントサイド完結 = データ漏洩リスクゼロの訴求

### 成功の罠

| 罠 | 対策 |
|----|------|
| 大手証券が類似ツール内製 | 税制精度 + 計算透明性で差別化。大手はブラックボックス傾向 |
| 税制更新の責任増大 | 計算根拠書12セクション方式 — ユーザー自身が検証可能 |
| 世帯シミュレーション複雑性爆発 | スコープを「世帯の資産」に絞り、相続・贈与は別ツールに委譲 |

### 失敗モードレジストリ

| # | 失敗モード | 影響 | 検知方法 | 復旧手順 |
|---|-----------|------|---------|---------|
| 1 | 税制JSONの数値誤り | 高 | 国税庁公式との突合テスト | JSON修正 → patch release |
| 2 | テンプレートがFormState変更に追従せず | 中 | `applyTemplate()` 型チェック + E2E | version + マイグレーション |
| 3 | リブランド時の既存リンク切れ | 高 | GitHub repoリネーム自動リダイレクト | 404.htmlでリダイレクト |
| 4 | 年度跨ぎ税計算不整合 | 中 | 設計判断: 開始年度固定 | 免責条項に明記 |

### デザイン品質スコア (v2.0.1時点)

| 次元 | 評価 | v3.0目標 |
|------|------|---------|
| D1 情報アーキテクチャ | 7/10 | 9 (テンプレートセレクター) |
| D2 ビジュアル階層 | 8/10 | 8 |
| D3 インタラクション | 6/10 | 8 (スケルトンUI) |
| D4 アクセシビリティ | 7/10 | 9 (データテーブル代替) |
| D5 レスポンシブネス | 7/10 | 9 (タッチターゲット44px) |
| D6 一貫性 | 8/10 | 8 |
| D7 AIスロップ | 8/10 | 9 (テンプレートでDEFAULT依存解消) |
| **総合** | **7.3** | **8.6** |
